...
describe('Test SmartContract `Bsv20V2Token`', () => {
    let instance: Bsv20V2Token

    before(async () => {
        await Bsv20V2Token.loadArtifact()

        instance = new Bsv20V2Token(sha256(toByteString('hello world', true)))
        await instance.connect(getDefaultSigner())
    })

    it('should pass the public method unit test successfully.', async () => {
        const deployTx = await instance.deploy(1)
        console.log(`Deployed contract "Bsv20V2Token": ${deployTx.id}`)
